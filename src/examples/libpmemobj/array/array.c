...
static enum array_types get_type(const char *type_name)
{
...
static TOID(struct array_info) find_array(const char *name)
{
...
static int elm_constructor(PMEMobjpool *pop, void *ptr, void *arg)
{
...
static void print_int(struct array_info *info)
{
    TOID(int) array;
...
static void print_pmemoid(struct array_info *info)
{
    TOID(PMEMoid) array;
...
static void print_toid(struct array_info *info)
{
...
static void free_int(struct array_info *info)
{
    TOID(int) array;
    TOID_ASSIGN(array, info->array);
...
static void free_pmemoid(struct array_info *info)
{
	TOID(PMEMoid) array;
	TOID_ASSIGN(array, info->array);
	/*
...
static PMEMoid realloc_toid(PMEMoid *info, size_t prev_size, size_t size)
{
    TOID_ARRAY(TOID(struct array_elm)) array;
    TOID_ASSIGN(array, *info);

    pmemobj_zrealloc(pop, &array.oid, sizeof(TOID(struct array_elm)) * size, TOID_TYPE_NUM_OF(array));

    for (size_t i = prev_size; i < size; i++) {
        POBJ_NEW(pop, &D_RW(array)[i], struct array_elm, elm_constructor, &i);
...
static fn_realloc realloc_array[] = {NULL, realloc_int, realloc_pmemoid, realloc_toid};

static PMEMoid alloc_int(size_t size)
{
    // alloc persistent space
    TOID(int) array;
    POBJ_ALLOC(pop, &array, int, sizeof(int) * size, NULL, NULL);
...
static PMEMoid alloc_pmemoid(size_t size)
{
    TOID(PMEMoid) array;
    POBJ_ALLOC(pop, &array, PMEMoid, sizeof(PMEMoid) * size, NULL, NULL);
    if (TOID_IS_NULL(array)) {
        fprintf(stderr, "POBJ_ALLOC\n");
        return OID_NULL;
    }
    ...
    for (size_t i = 0; i < size; i++) {
        if (pmemobj_alloc(pop, &D_RW(array)[i], sizeof(struct array_elm), TOID_TYPE_NUM(PMEMoid), elm_constructor, &i)) {
            fprintf(stderr, "pmemobj_alloc\n");
        }
    }
...
static PMEMoid alloc_toid(size_t size)
{
    TOID_ARRAY(TOID(struct array_elm)) array;
    POBJ_ALLOC(pop, &array, TOID(struct array_elm), sizeof(TOID(struct array_elm)) * size, NULL, NULL);
...
static void do_realloc(int argc, char *argv[])
{
    if (argc != 2) {
        printf("usage: ./array <file-name> realloc" " <array-name> <size>\n");
        return;
    }
    size_t size = atoi(argv[1]);

    // check whether this arrary have existed.
    TOID(struct array_info) array_info = find_array(argv[0]);
    if (TOID_IS_NULL(array_info)) {
        printf("%s doesn't exist\n", argv[0]);
        return;
...
{
    if (argc != 3) {
        printf("usage: ./array <file-name> alloc <array-name>" "<size> <type>\n");
        return;
    }
    ...
    // array-name have existed in file-name, so firstly delete it.
    if (!TOID_IS_NULL(array_info)) {
        printf("%s have existed in file. So firstly delete this %s array \n", argv[0], argv[0]);
        POBJ_FREE(&array_info);
    }

    // malloc persistent space.
    POBJ_ZNEW(pop, &array_info, struct array_info);

    // init this persistent space.
    struct array_info *info = D_RW(array_info);
